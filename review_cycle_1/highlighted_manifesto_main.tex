...
\begin{figure}
    ...
    {\color{red} \caption[Run types, properties, capabilities and features.]{Solo- and community-benchmarks, workflows, and benchmarking system-enabled (\textit{bs}) benchmarks differ in run properties and their consequences in terms of capabilities and features, including whether: inputs, truths, hidden truths, metrics or run results are available for readers; code is free software, software environments are managed, and execution automated; there is a clear track of (human) contributors; results are run in comparable hardware; an entrypoint to results or result interpretation are provided. These run intrinsic properties have consequences in terms of features (i.e. transparency, forkability, availability for metaanalysis) and technical capabilities (i.e. software versioning or ability to run anonymously and locally (stealth mode). \label{fig:definitions}}
    }
\end{figure}
...
{\color{red} Collaborative benchmarking efforts enunciate field-specific challenges and provide a venue for \emph{communities} to address them. For instance, DREAM

GIAB

CASP

have successfully shown how collaborative benchmarks can help in moving fields forward.}
...
{\color{red}
Aside of task completion-tailored metrics, computing performance (i.e. wallclock, on-disk read and write operations), requirements (e.g. GPUs, minimum required memory) and their scalability depending on the input size (e.g. whether runtimes increase linearly, quadratically, etc) are key, and routine, benchmarking metrics. To measure these accurately, benchmarking systems ideally would provide means to: i, characterize the hardware running the benchmark, so methods runtimes are comparable if run on homogeneous systems;  ii, track performances (CPU or GPU usage, memory) when running each benchmark component; iii, introduce little overhead during the profiling (so profiling does not considerably slow down runtimes); iv, collect variability in performance by running each method several times; v, go beyond reporting average resources usage and provide them over execution time (e.g. so embarrasingly parallel algorithms can be told apart from largely single-cored runs with few parallelism bursts).
}
...
{\color{red}
Mature communities are key in sustaining and reflecting the existing challenge in a given field. For instance, CASP, a continuous benchmarking by nature, has a established community defining the critical assesments scope, input data, target definitions, and composite and normalized metrics to equally weight multiple performance scores \cite{simpkin2023tertiary}. At the continuous benchmark level, CASP keeps track of past performances and introduces metrics to evaluate possible artifacts, i.e. whether round progress could be arising from different target difficulty \cite{kryshtafovych2019critical}. To facilitate community contribution, we believe benchmarking systems can lower the barrier to contributing, hence making the contribution efforts less likely to be biased. 
}
